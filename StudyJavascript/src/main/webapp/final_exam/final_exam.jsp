...
// 		alert(id);
	...
	*/
	...
	function checkForm(fr) { // 함수 파라미터로 폼 객체 전달받기(document.fr = fr)
		// 각 조건이 만족하지 않을 때마다 false 리턴, 모두 만족하면 true 리턴
		if(!isCorrectPassword) { // isCorrectPassword == false
			alert("패스워드 4 ~ 8자리 필수!");
			fr.passwd.select();
			return false;
			...
			fr.jumin2.select();
			return false;
		} else if(isDuplicate) { // 아이디가 중복일 경우(또는 사용 불가능할 경우) isDuplicate == true
			alert("아이디 확인 필수!");
			fr.id.select();
			return false;
		}
...
<body>
	...
	<h1>회원가입</h1>
	<form action="a.jsp" method="get" name="fr" onsubmit="return checkForm(this)">
		<table border="1">
